...
export class InvoiceService {
  ...
  private calculateRideDuration(ride: any): string {
    if (!ride.startedAt || !ride.completedAt) return 'N/A';
...
Generated by RideShare Pro System
    ...
    return await QRCode.toDataURL(qrText, { width: 500, margin: 2 });
  }
  ...
  async generateInvoice(rideId: string): Promise<Buffer> {
    try {
      ...
      const rideDuration = ride.status === 'completed' ? this.calculateRideDuration(ride) : 'N/A';
            ...
            <table>
              ...
              <tr>
                <td>
                  ...
                  📞 ${user.userContact}<br>
                 <span style="text-transform: lowercase"> 📧 ${user.userEmail}</span>
                </td>
                <td>
    ...
    if (type === 'income') {
      ...
      rows = items
        ...
        <tr>
          ...
          <td style="text-transform: uppercase">${ride.distance?.toFixed(2) || 0} km</td>
      ...
      rows = items
        ...
        <tr>
          <td>${index + 1}</td>
          <td>${ride._id}</td>
          <td>${ride.bookedBy?.name || 'N/A'} ${ride.bookedBy?.contactNumber || 'N/A'} </td>
          <td>${ride.driver?.name || 'N/A'} ${ride.driver?.contactNumber || 'N/A'} </td>
    ...
    if (type === 'income') {
      ...
      summaryRows = `
        ...
        <tr class="total"><td colspan="9">STARTED</td><td>${summary.statusCounts?.['started'] || 0}</td></tr>
               <tr class="total"><td colspan="9">ARRIVED</td><td>${summary.statusCounts?.['arrived'] || 0}</td></tr>
